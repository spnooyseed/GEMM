...
#define checkCudaErrors(func)                                                  \
  {                                                                            \
    cudaError_t e = func;                                                      \
    if (e != cudaSuccess) {                                                    \
      printf("gemm_v0_baseline , %s , %d CUDA: %s\n", __FILE__, __LINE__,      \
...
int main(int argc, char **argv) {
  if (argc != 4) {
    printf("gemm_v0_baseline , usage: ./main [N] [K] [M]");
    exit(0);
  }
  ...
  printf("gemm_v0_baseline , My gemm Performance= %.2f GFlop/s, Time= %.3f "
         "msec, Size= %.0f Ops,\n",
  ...
  printf("gemm_v0_baseline , CuBlas Performance= %.2f GFlop/s, Time= %.3f "
         "msec, Size= %.0f Ops,\n",
  ...
  for (int i = 0; i < N * M; i++) {
    ...
    if (rel_err > eps) {
      printf("gemm_v0_baseline , Error! Matrix[%05d]=%.8f, ref=%.8f error term "
             "is > %E\n",
  ...
  printf("gemm_v0_baseline , %s\n", correct ? "Result= PASS" : "Result= FAIL");
  printf("gemm_v0_baseline , ratio= %f\n", gigaFlops[0] / gigaFlops[1]);
