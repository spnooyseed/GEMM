...
__global__ void reduce_v4_unroll_last_warp(float *g_idata, float *g_odata) {
...
int main() {
  ...
  for (int i = 1; i <= nIter; ++i) {
    reduce_v4_unroll_last_warp<<<grid, block>>>(input_device, output_device);
  }
  ...
  if (check(output_host, res, block_num)) {
    ...
    printf("reduce_v4_unroll_last_warp , use time = %.3fms , and bandwidth = "
           "%.3fGB/s\n",
           msecTotal, N * sizeof(float) * 1e-9 / (msecTotal * 1e-3f));
  } else {
    printf("reduce_v4_unroll_last_warp , this is error\n");
    for (int i = 0; i < block_num; ++i) {
      if (output_host[i] != res[i])
