...
__global__ void reduce_v3_idle(float *g_idata, float *g_odata) {
...
int main() {
  ...
  for (int i = 1; i <= nIter; ++i) {
    reduce_v3_idle<<<grid, block>>>(input_device, output_device);
  }
  ...
  if (check(output_host, res, block_num)) {
    ...
    printf("reduce_v3_idle , use time = %.3fms , and bandwidth = %.3fGB/s\n",
           msecTotal, N * sizeof(float) * 1e-9 / (msecTotal * 1e-3f));
  } else {
    printf("reduce_v3_idle , this is error\n");
    for (int i = 0; i < block_num; ++i) {
      if (output_host[i] != res[i])
